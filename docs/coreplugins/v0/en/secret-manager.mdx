...
| `secretmgr.policy.<name>` | JSON array of strings | Sorted, de-duplicated plugin names allowed to access `<name>` through IPC. |
...
Plugin-to-plugin management uses the Kernel's synchronous plugin message
transport. The target plugin name is `secret-manager`. Every payload is
UTF-8 JSON, and the Kernel supplies the authenticated sender name as the
message `source`.

The plugin supports these topics:

| Topic | Access rule |
| --- | --- |
| `secret-manager.secret.get` | In `allowed_plugins` |
| `secret-manager.secret.list` | In `allowed_plugins` |
| `secret-manager.secret.add` | Any plugin |
| `secret-manager.secret.update` | In `allowed_plugins` |
| `secret-manager.secret.delete` | In `allowed_plugins` |
...
The passphrase may be empty for an `identity` secret and is required for an
`scrypt` secret. On success, the reply's `message` contains the plaintext
directly:
...
The reply's `message` is a JSON string. It contains only metadata for secrets
whose `allowed_plugins` includes the authenticated source:
...
The JSON encoded inside `message` has the same metadata fields as the HTTP
list response. Secret values are never included.
...
The source is added to `allowed_plugins` automatically, even when it is not in
the submitted list. An empty passphrase selects `identity` encryption; a
non-empty passphrase selects `scrypt` encryption.

On success, `message` contains the JSON string
`{"success":true,"name":"database-password"}`. A secret with the same name
returns an error.

### Update a secret

The payload has the same fields as the add request. The source must already be
allowed to access the secret. The submitted `allowed_plugins` list replaces
the existing policy; the source is not added automatically during an update.
This means an update can intentionally revoke the source's future access.

Set `value` to `*` and leave `passphrase` empty to preserve the existing
ciphertext while changing metadata or the access policy. Supplying a
passphrase with this sentinel is rejected.

On success, `message` contains a JSON success object. An unknown secret or a
source that is not currently allowed returns an error.
...
The source must be allowed to access the secret. On success, the plugin
deletes the ciphertext, policy, and metadata parameters and returns the JSON
success object in `message`.

### Message errors

On failure, `error` contains the error description and no secret value is
returned:
...
The message interface rejects unsupported topics, invalid JSON, missing
requesting plugins, invalid secret names, unauthorized sources, missing
secrets, duplicate additions, unsupported encryption metadata, missing or
invalid `scrypt` passphrases, and persistence failures.

For the common message envelope and host-authenticated source behavior, see
the [service message guide](../../../arupa/v0/en/service/isc-message).
