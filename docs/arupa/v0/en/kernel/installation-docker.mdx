...
The container stores its configuration, service packages, and temporary service
...
- `services`, the directory for `.plg` service packages; and
- `tmp`, the directory used while the Kernel loads service packages.
...
`Listen` is the HTTP and Socket.IO address inside the container. `ServiceDir`
and `ServiceTempDir` are relative to `/data`, which is the container's working
directory.
...
## Edit configuration and add service packages

The named volume holds both the configuration and service packages. The
...
To add a downloaded service package, copy its `.plg` file into `/data/services`.
For example:
...
`services` directory beneath the `Mountpoint` reported by `docker volume
inspect arupa-data`. Restart the container after changing service files so the
...
option, it reuses the existing configuration and service data.

## Next steps

The Kernel is ready for service packages. Continue with the [Coreplugins
documentation](../../../../coreplugins/v0/en/index), then follow [Install a
service](./services#install-a-service).
