...
HTTP paths for itself — the paths behind login, the whole [Management
API](./api), and the Socket.IO transport:
...
A path without a trailing slash reserves that exact path only, matching the
collection-level endpoint for that capability (`GET /api/user`,
...
[Subtrees](#subtrees) for what that pattern shape means. These reservations
...
[Management API](./api) for how the enabled/disabled state itself is
enforced.
