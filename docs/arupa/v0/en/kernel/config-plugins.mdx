...
`Checksum` verifies the integrity of a plugin package before the Kernel
...
plugin. Use a per-plugin checksum in most cases. Although `Checksum` can be
set in `[Plugins.default]`, that value is inherited by every plugin without
its own non-empty checksum.
...
plugin is next loaded or started; they do not rewrite an already running plugin
process.
