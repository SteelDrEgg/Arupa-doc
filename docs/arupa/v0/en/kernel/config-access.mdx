...
Because an empty `Groups` list is public, there's no way to write "deny
...
rule's group list is evaluated. See [Route](./route) for the complete
...
See [Denying access](#denying-access) above for how to write a rule that
...
The `admin` user can pass the `/api/service/` host-route rule because they are
