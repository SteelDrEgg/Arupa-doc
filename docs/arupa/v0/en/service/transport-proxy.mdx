...
A proxy transport has an `id` and a `type` of `proxy`, like any other
transport, plus a nested `proxy` block naming the upstream target:

| Property | Meaning |
| --- | --- |
| `id` | Identifier used by routes to bind to this transport. |
| `proxy.network` | How the Kernel reaches the upstream: `inherited`, `unix`, or `tcp`. |
| `proxy.address` | Target address. Meaning depends on `network`. |
| `proxy.scheme` | `http` or `https`, for how the Kernel connects to the upstream. |
